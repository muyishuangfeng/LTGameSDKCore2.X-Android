...
public class RechargeManager {
     ...
     * @param listener 登录回调
     */
    public static void recharge(Activity activity, int target, OnRechargeListener listener) {
     ...
     * @param listener 回调
     */
    public static void recharge(Activity activity, int target, RechargeObject object, OnRechargeListener listener) {
        if (mManager != null) {
     ...
     * 开始支付
     */
    static void actionRecharge(Activity activity) {
        if (mManager != null) {
    ...
    public static class LifecycleManager implements LifecycleObserver {
        ...
        private void prepareRecharge(Activity activity, @Target.RechargeTarget int target,
                                     RechargeObject object, OnRechargeListener listener) {
...
//            }
        ...
        private void postRecharge(Activity activity) {
            ...
            if (currentTarget == -1) {
                mStateListener.onState(activity, RechargeResult.
                        failOf(LTGameError.CODE_COMMON_ERROR, LTGameError.make(LTGameError.CODE_COMMON_ERROR, "login target error")));
                return;
            }
            //没有设置支付回调
            if (mStateListener == null) {
                mStateListener.onState(activity,
                        RechargeResult.failOf(currentTarget,
                                LTGameError.make(LTGameError.CODE_COMMON_ERROR, "没有设置 login listener")));
                return;
            }
            //创建的 platform 失效
            if (GlobalPlatform.getCurrentPlatform() == null) {
                mStateListener.onState(activity,
                        RechargeResult.failOf(currentTarget,
                                LTGameError.make(LTGameError.CODE_COMMON_ERROR, "创建的 platform 失效")));
                return;
            }
     ...
     * 用于支付后回收资源
     */
    private static class OnRechargeStateRealize implements OnRechargeListener {

        OnRechargeListener mListener;

        OnRechargeStateRealize(OnRechargeListener listener) {
